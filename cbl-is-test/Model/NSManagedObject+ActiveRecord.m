...
+ (NSManagedObjectContext *)mainContext {
    return [(CITAppDelegate *)[UIApplication sharedApplication].delegate managedObjectContext];
}

+ (void)saveMainContext {
    if (![NSThread isMainThread]) {
        [(CITAppDelegate *)[UIApplication sharedApplication].delegate performSelectorOnMainThread:@selector(saveContext) withObject:nil waitUntilDone:NO];
    } else
        [(CITAppDelegate *)[UIApplication sharedApplication].delegate saveContext];
}
...
+ (NSEntityDescription *)entity
{
    ...
    return [NSEntityDescription entityForName:className inManagedObjectContext:[NSManagedObjectContext mainContext]];
}
...
{
    NSError *error = nil;
    NSArray *objects = [[NSManagedObjectContext mainContext] executeFetchRequest:fetchRequest error:&error];
    if (objects == nil) {
...
{
    NSError *error = nil;
    NSUInteger objectCount = [[NSManagedObjectContext mainContext] countForFetchRequest:fetchRequest error:&error];
...
{
    NSFetchRequest *fetchRequest = [self fetchRequest];
    return [[NSManagedObjectContext mainContext] countForFetchRequest:fetchRequest error:error];
}
...
+ (id)object
{
    id object = [[self alloc] initWithEntity:[self entity] insertIntoManagedObjectContext:[NSManagedObjectContext mainContext]];
    return object;
}
...
+ (id)findByPrimaryKey:(id)primaryKeyValue inContext:(NSManagedObjectContext *)context
{
    ...
    if (! predicate) {
        NSLog(@"Attempt to findByPrimaryKey for entity with nil primaryKeyAttribute. Set the primaryKeyAttributeName and try again! %@", self);
        return nil;
    }
...
{
    return [self findByPrimaryKey:primaryKeyValue inContext:[NSManagedObjectContext mainContext]];
}
