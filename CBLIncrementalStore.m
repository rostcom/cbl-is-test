...
- (NSArray *)newArrayForRelationship:(NSRelationshipDescription *)relationship referenceObject:(id)data
{
    NSMutableArray *array = [[NSMutableArray alloc] initWithCapacity:[data count]];
    for (NSString *referenceObject in data) {
        [array addObject:[self newObjectIDForEntity:relationship.destinationEntity referenceObject:referenceObject]];
    }
    return array;
}
        ...
        case NSKeyPathExpressionType: {
            ...
            if ([property isKindOfClass:[NSRelationshipDescription class]]) {
                ...
                if (!rel.isToMany)
                    ...
                    value = [self newArrayForRelationship:rel referenceObject:value];
            }
        }
            break;
